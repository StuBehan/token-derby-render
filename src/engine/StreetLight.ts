...
export class StreetLight {
  ...
  private buildModel(material: THREE.Material) {
    ...
    // 1. Ornate Base
    const base = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.36, 0.8, 8), material);
    base.position.y = 0.4;
    ...
    const molding = new THREE.Mesh(new THREE.CylinderGeometry(0.26, 0.28, 0.12, 8), material);
    molding.position.y = 0.8;
    molding.castShadow = true;
    this.group.add(molding);

    // 2. Main Tapered Shaft
    const shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.20, 4.4, 8), material);
    shaft.position.y = 3.0; // centered at 3.0 (from 0.8 to 5.2)
    shaft.castShadow = true;
    this.group.add(shaft);

    // 3. Ornate Rings
    const ring1 = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.1, 8), material);
    ring1.position.y = 2.0;
    ring1.castShadow = true;
    this.group.add(ring1);

    const ring2 = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.1, 8), material);
    ring2.position.y = 4.0;
    ring2.castShadow = true;
    this.group.add(ring2);

    // 4. Pole Top Cap
    const topCap = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.3, 8), material);
    ...
    for (const side of [-1, 1]) {
      const brace = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.54, 0.08), material);
    ...
    for (const side of [-1, 1]) {
      const finial = new THREE.Mesh(new THREE.SphereGeometry(0.08, 6, 6), material);
      finial.position.set(side * 0.96, 5.4, 0);
      this.group.add(finial);
    }

    // A central top spire
    const spire = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.4, 6), material);
    ...
    for (const side of [-1, 1]) {
      ...
      const loop = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.12, 0.06), material);
      loop.position.y = 0.05;
      lanternGroup.add(loop);

      // Lantern Top Roof (cap)
      const roof = new THREE.Mesh(new THREE.ConeGeometry(0.32, 0.18, 6), material);
      roof.position.y = -0.06;
      roof.castShadow = true;
      lanternGroup.add(roof);

      // Glass Body (hexagonal prism)
      const bulb = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.22, 0.34, 6), glassMaterial);
      bulb.position.y = -0.32;
      bulb.castShadow = true;
      lanternGroup.add(bulb);

      // Lantern Bottom Cap
      const bottomCap = new THREE.Mesh(new THREE.CylinderGeometry(0.20, 0.06, 0.1, 6), material);
      bottomCap.position.y = -0.52;
      bottomCap.castShadow = true;
      lanternGroup.add(bottomCap);

      const bottomFinial = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.12, 6), material);
      bottomFinial.position.y = -0.62;
      bottomFinial.rotation.x = Math.PI; // point downwards
      bottomFinial.castShadow = true;
